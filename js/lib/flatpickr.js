...
((global, factory) => {
    ...
    ***************************************************************************** */
	...
	var doNothing = () => undefined;
	var monthToStr = (monthNumber, shorthand, locale) =>
	...
	var revFormat = {
		...
		G: (dateObj, hour) => {
			dateObj.setHours(
				(dateObj.getHours() >= 12 ? 12 : 0) + Number.parseFloat(hour),
			);
		},
		H: (dateObj, hour) => {
			dateObj.setHours(Number.parseFloat(hour));
		},
		J: (dateObj, day) => {
			dateObj.setDate(Number.parseFloat(day));
		},
		...
		S: (dateObj, seconds) => {
			dateObj.setSeconds(Number.parseFloat(seconds));
		},
		U: (_, unixSeconds) => new Date(Number.parseFloat(unixSeconds) * 1000),
		W: (dateObj, weekNum, locale) => {
			var weekNumber = Number.parseInt(weekNum);
			var date = new Date(
				dateObj.getFullYear(),
				0,
				2 + (weekNumber - 1) * 7,
		...
		Y: (dateObj, year) => {
			dateObj.setFullYear(Number.parseFloat(year));
		},
		...
		i: (dateObj, minutes) => {
			dateObj.setMinutes(Number.parseFloat(minutes));
		},
		j: (dateObj, day) => {
			dateObj.setDate(Number.parseFloat(day));
		},
		l: doNothing,
		m: (dateObj, month) => {
			dateObj.setMonth(Number.parseFloat(month) - 1);
		},
		n: (dateObj, month) => {
			dateObj.setMonth(Number.parseFloat(month) - 1);
		},
		...
		u: (_, unixMillSeconds) => new Date(Number.parseFloat(unixMillSeconds)),
		w: doNothing,
		y: (dateObj, year) => {
			dateObj.setFullYear(2000 + Number.parseFloat(year));
		},
	};
	var tokenRegex = {
		D: "",
	...
	var formats = {
		...
		D: (date, locale, options) =>
			locale.weekdays.shorthand[formats.w(date, locale, options)],
		// full month name e.g. January
		F: (date, locale, options) =>
			monthToStr(formats.n(date, locale, options) - 1, false, locale),
		// padded hour 1-12
		G: (date, locale, options) => pad(formats.h(date, locale, options)),
		// hours with leading zero e.g. 03
		H: (date) => pad(date.getHours()),
		// day (1-30) with ordinal suffix e.g. 1st, 2nd
		J: (date, locale) =>
			locale.ordinal !== undefined
				? date.getDate() + locale.ordinal(date.getDate())
				: date.getDate(),
		// AM/PM
		K: (date, locale) => locale.amPM[int(date.getHours() > 11)],
		// shorthand month e.g. Jan, Sep, Oct, etc
		M: (date, locale) => monthToStr(date.getMonth(), true, locale),
		// seconds 00-59
		S: (date) => pad(date.getSeconds()),
		// unix timestamp
		U: (date) => date.getTime() / 1000,
		W: (date, _, options) => options.getWeek(date),
		...
		l: (date, locale) => locale.weekdays.longhand[date.getDay()],
	...
	var isBetween = (ts, ts1, ts2) =>
		ts > Math.min(ts1, ts2) && ts < Math.max(ts1, ts2);
	...
	function FlatpickrInstance(element, instanceConfig) {
		...
		function setHoursFromInputs() {
			if (self.hourElement === undefined || self.minuteElement === undefined)
				return;
			var hours =
					(Number.parseInt(self.hourElement.value.slice(-2), 10) || 0) % 24,
				minutes = (Number.parseInt(self.minuteElement.value, 10) || 0) % 60,
				seconds =
					self.secondElement !== undefined
						? (Number.parseInt(self.secondElement.value, 10) || 0) % 60
						: 0;
		...
		function onYearInput(event) {
			...
			var year = Number.parseInt(eventTarget.value) + (event.delta || 0);
			if (
				year / 1000 > 1 ||
		...
		function bindEvents() {
			...
			if (
				...
				var selText = (e) => getEventTarget(e).select();
		...
		function buildMonth() {
			...
			if (
				...
				bind(self.monthsDropdownContainer, "change", (e) => {
					...
					var selectedMonth = Number.parseInt(target.value, 10);
		...
		function documentClick(e) {
			if (self.isOpen && !self.config.inline) {
				...
				var isIgnored = !self.config.ignoredFocusElements.some((elem) =>
					elem.contains(eventTarget_1),
				);
		...
		function parseConfig() {
			...
			for (var i = 0; i < boolOpts.length; i++)
				...
				self.config[boolOpts[i]] =
					...
					self.config[boolOpts[i]] === "true";
			HOOKS.filter((hook) => self.config[hook] !== undefined).forEach(
				(hook) => {
		...
		function selectDate(e) {
			...
			if (self.config.mode === "single") self.selectedDates = [selectedDate];
				...
				if (selectedIndex)
					self.selectedDates.splice(Number.parseInt(selectedIndex), 1);
		...
		function timeWrapper(e) {
			...
			var min = Number.parseFloat(input.getAttribute("min")),
				max = Number.parseFloat(input.getAttribute("max")),
				step = Number.parseFloat(input.getAttribute("step")),
				curValue = Number.parseInt(input.value, 10),
